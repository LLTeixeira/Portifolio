...
const IEEEFloat = Union{Float16, Float32, Float64, Float128}
...
significand_mask(::Type{Float128})= 0xffff_ffff_ffff_ffff_ffff_ffff_ffff
...
promote_rule(::Type{Float128}, ::Type{UInt128}) = Float64
promote_rule(::Type{Float128}, ::Type{Int128}) = Float64
...
function Float128(x::UInt128)
    ...
    if n <= 113
        y = ((x % UInt128) << (113-n)) & 0xffff_ffff_ffff_ffff_ffff_ffff_ffff
    else
...
function Float128(x::Int128)
    x == 0 && return 0.0
    s = ((x >>> 64) % UInt128) & 0x8000_0000_0000_0000_0000_0000_0000_0000 # sign bit
    x = abs(x) % UInt128
    n = 128-leading_zeros(x) # ndigits0z(x,2)
    if n <= 113
        y = ((x % UInt128) << (113-n)) & 0xffff_ffff_ffff_ffff_ffff_ffff_ffff
...
for Ti in (Int64,UInt64,Int128,UInt128)
    for Tf in (Float32,Float64,Float128)
        @eval begin
            function ==(x::$Tf, y::$Ti)
                fy = ($Tf)(y)
...
function decompose(x::Float128)::Tuple{Int128, Int, Int}
    ...
    s = (n & 0xffffffffffffffffffffffffffff) % Int128
    e = ((n & 0x7fff0000000000000000000000000000) >> 112) % Int
    s |= Int128(e != 0) << 112
    d = ifelse(signbit(x), -1, 1)
    s, e - 1075 + (e == 0), d
end
